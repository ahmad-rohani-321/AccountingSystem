        ...
        IWebHostEnvironment env) : ApiControllerBase
    {
        ...
        public async Task<IActionResult> SaveInitialStock(int id, [FromBody] List<InitialStockRowVm> rows)
        {
            ...
            foreach (var row in rows)
            {
                ...
                var checkItem = await _db.StockBalances.FirstOrDefaultAsync(x => x.ItemID == id && x.WarehouseID == row.WarehouseID);

                if (checkItem != null)
                {
                    // do update
                    checkItem.Quantity += (row.Quantity / exchangedAmount);
                    await _db.SaveChangesAsync();

                    var trx = new StockTransactions
                    {
                        StockBalanceID = checkItem.ID,
