...
namespace AccountingSystem.Controllers.APIs
{
    ...
    [ApiController]
    [Authorize]
    public class CategoriesController(IHttpContextAccessor accessor, ICategoryRepository category) : ControllerBase
    {
        ...
        private static void ApplyValues(Category entity, string values)
        {
            ...
            if (dict.TryGetValue(nameof(Category.Name), out var nameEl) && nameEl.ValueKind != JsonValueKind.Null)
                entity.Name = nameEl.GetString() ?? string.Empty;

            if (dict.TryGetValue(nameof(Category.Description), out var descEl) && descEl.ValueKind != JsonValueKind.Null)
                entity.Description = descEl.GetString() ?? string.Empty;

            if (dict.TryGetValue(nameof(Category.IsActive), out var activeEl) && activeEl.ValueKind is JsonValueKind.True or JsonValueKind.False)
