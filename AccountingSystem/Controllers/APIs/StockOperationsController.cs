...
[ApiController]
[Authorize]
public class StockOperationsController(ApplicationDbContext db) : ApiControllerBase
{
    ...
    public async Task<IActionResult> SaveRemainingStock([FromBody] RemainingStockOperationVm request)
    {
        ...
        catch
        {
            ...
            return StatusCode(500, new { errors = new { Save = new[] { "د ثبت پر مهال خطا وسوه. بیا هڅه وکړئ." } } });
        }
        finally
        {
            if (tx is not null)
